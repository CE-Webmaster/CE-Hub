...
//                $ip = '72.229.28.185';//NY
            ...
            $isNoAutoMethod = !Mage::getStoreConfig('amautoshipping/general/shipping_method') && Mage::getStoreConfig('amautoshipping/general/select_method') == 'not_autoselect';
    ...
    protected function _addShippingMethod($quote, $isMethodInQuote = false)
    {
        ...
        $shippingAddress = $quote->getShippingAddress();
        $allShippingRates = $quote->getShippingAddress()->getAllShippingRates();
    ...
    protected function _needAutoshipping()
    {
        ...
        $backtrace = debug_backtrace();
        foreach ($backtrace as $step) {
            if ((isset($step['object']) && ($step['object'] instanceof Mage_Checkout_CartController || $step['object'] instanceof Mage_Checkout_OnepageController))
                && ($step['function'] == 'indexAction' || $step['function'] == 'addAction')
            ) {
