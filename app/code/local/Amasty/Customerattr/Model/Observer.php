...
class Amasty_Customerattr_Model_Observer
{
    ...
    public function handleBlockOutput($observer)
    {
        ...
        $onepageBillingClass = 'Uni_Opcheckout_Block_Onepage';
        ...
        if (Mage::getStoreConfig(
            ...
            if ($onepageBillingClass == get_class($block)
                ...
                $flag = true;
                if (false !== strpos($html, '<!--customer attributes-->')) {
//                    if ($block->isCustomerLoggedIn()) {
//                        $pos = strpos($html, '<div class="buttons-set"') - 1;
//                    } else {
//                        $pos = strpos($html,'<li class="fields" id="register-customer-password">');// + 51;
//                    }
//                    $pos = strpos($html, '<div id="billing-imformation-fields"') - 1;
                    $pos = strripos($html,'<!--customer attributes-->');
                    $insert = Mage::helper('amcustomerattr')->fields();
                    $html = substr_replace($html, $insert, $pos, 0);
                    $html = str_replace('<!--customer attributes-->','',$html);
                }
            }
        ...
        if ($massactionClass == get_class($block)
            ...
            $block->addItem(
                ...
                array(
                    ...
                    'url'     => Mage::helper("adminhtml")->getUrl(
                        'amcustomerattr/adminhtml_activation/massDeactivate'
                    ),
            ...
            $block->addItem(
                ...
                array(
                    ...
                    'url'     => Mage::helper("adminhtml")->getUrl(
                        'amcustomerattr/adminhtml_activation/massActivate'
                    ),
                    'confirm' => Mage::helper('amcustomerattr')->__(
                        'Are you sure?'
                    ))
