...
class Clevelandequipment_Commercebug_Helper_Formatlog_Allsimple extends Clevelandequipment_Commercebug_Helper_Data
{
        ...
        if(isset($object->observers))
        {
            foreach($object->observers as $item)
            {
                foreach(get_object_vars($item) as $name=>$props)
                {
                    ...
                    #Clevelandequipment_Commercebug_Model_Shim::Log('Mage2 refactoring needed at ' . __CLASS__ . ' ' . __LINE__);
                    unset($info['config']);
                    $result .= implode("\t", $info) . "\n";
                }                
