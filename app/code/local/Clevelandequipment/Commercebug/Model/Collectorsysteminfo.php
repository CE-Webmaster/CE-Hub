...
*/
class Clevelandequipment_Commercebug_Model_Collectorsysteminfo extends Clevelandequipment_Commercebug_Model_Observingcollector
{
    protected $_items;
    public function collectInformation($observer)
    {
        ...
        {
            Clevelandequipment_Commercebug_Model_Crossareaajax::validateControllers();
        }
        
        $system_info->ajax_paths = new stdClass;
        $system_info->form_key = $this->getShim()->getSingleton('core/session')->getFormKey();
        foreach(Clevelandequipment_Commercebug_Model_Crossareaajax::getAjaxActionNames() as $action)
        {
    ...
    public function createAjaxUrl($action)
    {
        ...
        #Clevelandequipment_Commercebug_Model_Shim::Log('Mage2 Cleanup in ' . __CLASS__ . ' ' . __LINE__);
    ...
    */		
    public function getUrl()
    {
        ...
        #Clevelandequipment_Commercebug_Model_Shim::Log('Cleanup in ' . __CLASS__ . ' ' . __LINE__);
        if($alias=='adminhtml/url' && $shim->isMage2())
        {
            $alias = 'backend/url';
        }
